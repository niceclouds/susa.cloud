...
function renameImageToSize(input, pattern) {
  return new Promise((resolve, reject) => {
    ...
    if (!image) {
      return reject('Unable to open image file');
    }

    if (!pattern) {
      return reject('Pattern missing');
    }
    ...
    image.metadata().then(metadata => {
      ...
      const renamePortion =
        execResult && execResult.length > 0 ? execResult[1] : null;

      if (!renamePortion) {
        debug(
          `Pattern did not match file ${input}, not doing anything for this file`
        );
        resolve();
      }

      const newName = imagePath.name.replace(renamePortion, newNamePortion);
      const newPath = Object.assign({}, imagePath, {
        name: newName,
        base: `${newName}${imagePath.ext}`
      });
